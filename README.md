...
1.  **Feature Extraction:** Extracts acoustic features from audio files using the **Librosa** library.
2.  **Dimensionality Reduction:** Reduces the feature space using **Principal Component Analysis (PCA)**.
3.  **Clustering:** Applies the **K-Means** algorithm to group the audio samples into distinct clusters.
4.  **Visualization:** Displays the results in an interactive **Streamlit** application.
...
* Download `Data.zip` from this Kaggle page: [https://www.kaggle.com/datasets/andradaolteanu/gtzan-dataset-music-genre-classification](https://www.kaggle.com/datasets/andradaolteanu/gtzan-dataset-music-genre-classification).
* Unzip `Data.zip`.
